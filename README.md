...
1. 搭建测试环境时，建议使用外网IP，配置 [openEuler 官方 repo](https://gitee.com/link?target=https%3A%2F%2Frepo.openeuler.org%2F) 中对应版本的 everything 和 update repo源，以下为参考配置。
...
目前通过`dnf install oec-hardware`安装的测试工具问题太多，无法完成测试，建议自行编译安装，官方步骤如下
...
- cpupower frequency-info 可以查看目前支持5种模式。
  ...
  2. 如果在测试过程中遇到Test userspace failed.错误，请检查测试工具是否通过dnf install oec-hardware安装，如果是，请卸载后自行编译安装。
...
1. 如果使用 `dnf` 安装客户端 oec-hardware。	`dnf install oec-hardware	`。在进行Ethernet测试时需要将network.py文件中的代码
...
中的`cmd = "ping -q -c %d -i 0 %s | grep 'packet loss' | awk '{print $6}'" % (count, self.server_ip)`改为`cmd = "ping -q -c %d -i 0.001 %s | grep 'packet loss' | awk '{print $6}'" % (count, self.server_ip)`。否则测试会报错。
